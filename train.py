...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create, nc = 80 ,
    ...
    # SyncBatchNorm - this is to solve multi-gpus training
    ...
    hyp['box'] *= 3 / nl  # scale to layers ， 1
    hyp['cls'] *= nc / 80 * 3 / nl  # scale to classes and layers ， 1
    hyp['obj'] *= (imgsz / 640) ** 2 * 3 / nl  # scale to image size and layers ， 1
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.image_weights: # this is ignored.
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional) - evoluation algorithm like genetic algorithm
    else:
